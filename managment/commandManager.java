...
public class commandManager implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
		...
		case "end": {
			...
			player.sendMessage(prefix + "The number was " + ChatColor.AQUA + guessManager.getRandomNumber(player));
			break;
		}
		case "min": {
			if (args.length == 1) {
			...
			try {
				...
				guessManager.setGuessMin(player, i);
		...
		default:
			if (guessManager.isGuessing(player)) {
				...
				try {
					...
					player.sendMessage(prefix + "The number " + ChatColor.AQUA + Integer.valueOf(args[0]) + ChatColor.GOLD + " is not in the range from " + ChatColor.AQUA + guessManager.getGuessMin(player) + ChatColor.GOLD + "-" + ChatColor.AQUA + guessManager.getGuessMax(player));
					break;
				} catch (NumberFormatException e) {
					sendUsage(player);
					break;
